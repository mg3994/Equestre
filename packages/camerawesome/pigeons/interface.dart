...
// Horse info API
// This API is used to update the horse info in the app.
class HorseInfo {
  final String name;
  final int number;
  final String rider;
...
@HostApi()
abstract class HorseApi {
  void updateHorseInfo(HorseInfo horse);
}
// Live match status API
// This API is used to update the live match status in the app.
class LiveMatchStatus {
  final bool isLive;
  final String? message; // Optional (e.g., "Match started", "No match today")
...
@HostApi()
abstract class LiveMatchApi {
  void updateLiveStatus(LiveMatchStatus status);
}

class PenaltyInfo {
  final double value; // e.g., 4.0
  final String description; // e.g., "penalties"
...
class TimeInfo {
  final double seconds; // e.g., 25.50
  final String display; // e.g., "25.50s"
...
class RankInfo {
  final int position; // e.g., 10
  final String? label; // e.g., "Rank 10"
...
class GapToBestInfo {
  final double gapSeconds; // e.g., -1.23
  final bool isFaster;     // true if negative gap
  final String? display;   // e.g., "-1.23s to best"
...
@HostApi()
abstract class PenaltyApi {
  void updatePenalty(PenaltyInfo penalty);
}
...
abstract class RankApi {
  void updateRank(RankInfo rank);
}

@HostApi()
abstract class GapToBestApi {
  void updateGapToBest(GapToBestInfo gap);
}
//////////////// 
class PreviewSize {
  final double width;
